...
It also ships in two shareable forms:

| File | What it is |
| --- | --- |
| [`summary.docx`](summary.docx) | Word conversion of `docs/SUMMARY.md` — same prose, same order |
| [`summary.pdf`](summary.pdf) | Separate **visual guide**: 11-page A4 layout with a colour-coded stage flow, model cards and comparison panels |

Both are regenerated by the scripts in [`tools/`](tools/) — see [`tools/README.md`](tools/README.md).
The PDF is hand-designed rather than converted, so content changes belong in both it and the markdown.
